...
What the package does: The GLARMA package takes data and models changes in population against explanatory variables. It is incredibly useful for time series data that is not normal. 
...
Here we're working with Generalized Linear Autoregressive Moving Average Models, or GLARMA. Mostly going to be focusing on Poisson. 

Need to add an intercept and needs to be a matrix. 
...
One vector: observational count data.

Time is needed. Can vary the time lag. 
...
You can choose whether or not to factor major events into these models, including if you want to compare them to any models without. Also has to be a matrix. The major events used in this example are the reintroduction of wolves to Yellowstone and the 1988 Yellowstone Fire. 
...
Using a time lag of 2 since that was the best fit model.
