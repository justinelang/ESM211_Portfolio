...
This is a lot of wrangling specific to this dataset (and making it more manageable):
...
In this instance, $r = 0.30$. It's a bit higher. 
...
```{r}
p <- c(y0 = 0.01, mumax = 0.2, K = 0.1) ### sort of have to eyeball it
...
$r = 0.20$ and $K = 0.10$. Sanity check suggests this makes sense. 
...
It's clear why this package is often used for bacteria.
...
Very simple to display many graphs with only a few lines of code.
...
Switching from bacteria to bison:

```{r}
### Read in bison data

bison <- read_csv(here("growthrates", "bison.csv")) %>% 
  mutate(time = year - year[1]) ### remember time has to start from 0
```

Starting with `fit_easylinear`

```{r}
bison_fit <- fit_easylinear(bison$time, bison$bison)
...
```{r}
par(mfrow = c(1, 2))
plot(bison_fit, log = "y", cex.lab = 2, cex.axis = 2)
plot(bison_fit, cex.lab = 2, cex.axis = 2)
```

Not the greatest fit.

Moving on to `fit_spline`

```{r}
bison_res <- fit_spline(bison$time, bison$bison)

coef(bison_res)
```

$r = 0.51$. That's a bit of a difference. 

```{r}
par(mfrow = c(1, 2))
plot(bison_res, log = "y", cex.lab = 2, cex.axis = 2)
plot(bison_res, cex.lab = 2, cex.axis = 2)
```

Visually, I would call that an overestimation of r. 

Let's try `fit_growthmodel`

```{r}
bison_p <- c(y0 = 484, mumax = 0.2, K = 4114) ### making my best guess best on the graphs

bison_fit1 <- fit_growthmodel(FUN = grow_logistic, 
                        p = bison_p, 
                        bison$time, 
                        bison$bison)
...
$r = 0.18$ and $K = 3742$. Not terrible. r makes a little more sense with this model. K seems like it could be a bit of an underestimation. 

```{r}
plot(bison_fit1, cex.lab = 2, cex.axis = 2)
```

Visually, looks like a better fit than the other two.

Useful package. Definitely have to be familiar with your data before deciding on which model you want to use, though that is good practice regardless. 
